...
const PayslipViewModal = ({ isOpen, onClose, payslip }) => {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-5xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="border rounded-xl p-4">
              ...
              <div className="space-y-2">
                ...
                {payslip.other_deductions_description && (
                  <div className="flex justify-between text-xs text-gray-500">
                    <span>{payslip.other_deductions_description}</span>
                  </div>
                )}
                <div className="flex justify-between pt-2 border-t font-semibold">
          ...
          {/* Attendance Summary */}
          {(payslip.days_absent > 0 || payslip.days_late > 0) && (
            <div className="mt-4 bg-yellow-50 rounded-xl p-4">
              <h4 className="text-sm font-medium text-yellow-800 mb-2">Attendance Summary</h4>
              <div className="grid grid-cols-3 gap-4 text-center">
                <div>
                  <p className="text-xs text-yellow-600">Days Present</p>
                  <p className="font-semibold text-yellow-800">{formatNumber(payslip.days_present)}</p>
                </div>
                <div>
                  <p className="text-xs text-yellow-600">Days Absent</p>
                  <p className="font-semibold text-yellow-800">{formatNumber(payslip.absences_days)}</p>
                </div>
                <div>
                  <p className="text-xs text-yellow-600">Days Late</p>
                  <p className="font-semibold text-yellow-800">{formatNumber(payslip.days_late)}</p>
                </div>
              </div>
            </div>
          )}
