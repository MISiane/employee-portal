...
const HighFiveButton = ({ employeeId, employeeName, onHighFiveSuccess }) => {
  ...
  const handleHighFive = async () => {
    ...
    try {
      ...
      alert(error.response?.data?.error || 'Failed to send props. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <button
      ...
      title={hasHighFived ? `Already high-fived ${employeeName}` : `High-five ${employeeName}`}
    >
      {loading ? (
...
const LeaderboardModal = ({ isOpen, onClose, employees }) => {
      ...
      <div className="bg-white rounded-2xl max-w-md w-full max-h-[80vh] overflow-hidden" onClick={(e) => e.stopPropagation()}>
        <div className="bg-gradient-to-r from-amber-500 to-yellow-500 p-4 text-white">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-xl font-bold">Props Leaderboard</h3>
            </div>
            <button onClick={onClose} className="text-white/80 hover:text-white text-2xl leading-none">
              ×
            </button>
          </div>
          <p className="text-white/80 text-sm mt-1">Most appreciated colleagues of all time</p>
        </div>
        ...
        <div className="p-4 overflow-y-auto max-h-[60vh]">
          {loading ? (
            ...
            <div className="text-center py-8 text-gray-500">
              ...
              <p className="text-sm mt-1">Be the first to appreciate a colleague!</p>
            </div>
          ) : (
            <div className="space-y-2">
              {leaderboard.map((person, index) => {
                const medal = index === 0 ? '🥇' : index === 1 ? '🥈' : index === 2 ? '🥉' : null;
                const medalColor = index === 0 ? 'from-yellow-400 to-amber-500' 
                ...
                return (
                  <div 
                    ...
                    className={`flex items-center justify-between p-3 rounded-xl transition ${
                      index < 3 ? 'bg-gradient-to-r ' + medalColor + ' bg-opacity-20' : 'bg-gray-50'
                    }`}
                  >
                    <div className="flex items-center gap-3">
                      ...
                      {person.avatar_url ? (
                        ...
                        <div className={`h-10 w-10 rounded-full bg-gradient-to-r ${medalColor} flex items-center justify-center`}>
                    ...
                    <div className="flex items-center gap-1 bg-white/50 rounded-full px-3 py-1">
                      ...
                      <span className="text-xs text-gray-400">props</span>
                    </div>
                  </div>
                );
              })}
        ...
        <div className="p-4 border-t bg-gray-50">
          <p className="text-xs text-center text-gray-500">
            ✨ Each prop is a unique appreciation from a colleague. You can only prop someone once!
          </p>
        </div>
      </div>
    </div>
...
const [filterUpcomingAnniversaries, setFilterUpcomingAnniversaries] = useState(false); 
...
const getUpcomingAnniversaries = (employeesList) => {
  ...
  const anniversariesWithDates = employeesList.map(emp => {
    if (!emp.hire_date) return null;
    
    const hireDate = new Date(emp.hire_date);
    // Get anniversary this year
    let nextAnniversary = new Date(currentYear, hireDate.getMonth(), hireDate.getDate());
    
    // If anniversary already passed this year, use next year
    if (nextAnniversary < today) {
      nextAnniversary = new Date(currentYear + 1, hireDate.getMonth(), hireDate.getDate());
    }
    
    const daysUntil = Math.ceil((nextAnniversary - today) / (1000 * 60 * 60 * 24));
    const yearsAtAnniversary = currentYear - hireDate.getFullYear() + (nextAnniversary > today ? 0 : 1);
...
const getSortedEmployees = () => {
  let filtered = employees.filter(emp => {
    ...
    const matchesAnniversary = !filterUpcomingAnniversaries || upcomingAnniversaries.some(a => a.id === emp.id);
    return matchesSearch && matchesDept && matchesBirthday && matchesAnniversary;
  });
...
{(upcomingBirthdays.length > 0 || upcomingAnniversaries.length > 0) && !filterUpcomingBirthdays && !filterUpcomingAnniversaries && (
  <div className="bg-gradient-to-r from-pink-50 to-purple-50 rounded-xl p-4 border border-pink-100">
    <div className="flex items-center gap-2 mb-3">
      <SparklesIcon className="h-5 w-5 text-purple-500" />
      <h3 className="font-semibold text-gray-800">Upcoming Celebrations 🎉</h3>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {upcomingBirthdays.slice(0, 3).map(emp => (
              <div key={`bday-${emp.id}`} className="flex items-center gap-2 bg-white rounded-lg px-3 py-2 shadow-sm hover:shadow-md transition cursor-pointer" onClick={() => setSelectedEmployee(emp)}>
                {emp.avatar_url ? (
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">
                    {formatBirthday(emp.date_of_birth)} • {emp.daysUntil} day{emp.daysUntil !== 1 ? 's' : ''} away
                  </p>
                </div>
              </div>
            ))}
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {upcomingAnniversaries.slice(0, 3).map(emp => {
              const rank = getTenureRank(emp.years_at_company, emp.months_at_company);
              const yearText = emp.yearsAtAnniversary === 1 ? '1 year' : `${emp.yearsAtAnniversary} years`;
              return (
                <div key={`anniv-${emp.id}`} className="flex items-center gap-2 bg-white rounded-lg px-3 py-2 shadow-sm hover:shadow-md transition cursor-pointer" onClick={() => setSelectedEmployee(emp)}>
                  {emp.avatar_url ? (
                    <img src={emp.avatar_url} className="h-8 w-8 rounded-full object-cover" />
                  ) : (
                  ...
                  <div>
                    <p className="text-sm font-medium">{emp.first_name} {emp.last_name}</p>
                    <p className="text-xs text-gray-500">
                      🏆 {yearText} • {rank.title} • {emp.daysUntil} day{emp.daysUntil !== 1 ? 's' : ''} away
                    </p>
                  </div>
                </div>
              );
  ...
  <div className="flex items-center justify-between bg-blue-50 rounded-lg px-3 py-2">
    <span className="text-sm text-blue-700">🏆 Showing employees with upcoming work anniversaries</span>
    <button onClick={() => setFilterUpcomingAnniversaries(false)} className="text-blue-600 hover:text-blue-800 text-sm">
      Clear filter →
    </button>
  </div>
)}
...
{upcomingAnniversaries.some(a => a.id === emp.id) && (
  <div className="mt-2 text-xs text-blue-600 flex items-center gap-1">
    ...
    {(() => {
      const anniv = upcomingAnniversaries.find(a => a.id === emp.id);
      return anniv && <span>{anniv.yearsAtAnniversary} year{anniv.yearsAtAnniversary !== 1 ? 's' : ''} 🏆</span>;
    })()}
  </div>
)}
            </div>
...
>
  <span>👏</span>
  <span className="font-semibold">{selectedEmployee.high_five_count|| 0}</span>
</button>
          ...
          <HighFiveButton 
            ...
            onHighFiveSuccess={(newCount) => {
              setSelectedEmployee(prev => ({ ...prev, high_five_count: newCount }));
      ...
      <div className="p-6 space-y-3">
        <div className="flex justify-between py-2 border-b">
          ...
          <span className="font-medium">{selectedEmployee.employee_code || 'N/A'}</span>
        </div>
        
        <div className="flex justify-between py-2 border-b">
          <span className="text-gray-500">Time with the Company</span>
          <div className="flex flex-col items-end gap-1">
            <span className="font-medium">{selectedEmployee.tenure || 'Less than 1 month'}</span>
            {selectedEmployee.tenure && (() => {
              const rank = getTenureRank(selectedEmployee.years_at_company, selectedEmployee.months_at_company);
              return (
                <span className={`inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full ${rank.bgColor} ${rank.textColor}`}>
                  <span>{rank.emoji}</span>
                  <span>{rank.title}</span>
                </span>
              );
        ...
        {upcomingAnniversaries.some(a => a.id === selectedEmployee.id) && (() => {
          const anniv = upcomingAnniversaries.find(a => a.id === selectedEmployee.id);
          return (
            <div className="bg-blue-50 rounded-lg p-3 text-center">
              <p className="text-blue-600 font-medium text-sm">
                🏆 Work anniversary soon! {anniv.yearsAtAnniversary} year{anniv.yearsAtAnniversary !== 1 ? 's' : ''} with us!
              </p>
            </div>
          );
        })()}
